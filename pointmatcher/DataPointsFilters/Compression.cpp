	...
	while(tempCloud.getNbPoints() != cloud.getNbPoints() && iterationCount++ < maxIterationCount)
	{
		...
		for(unsigned i = 0; i < tempCloud.getNbPoints(); ++i)
		{
			if(masks(0, i))
			{
				...
				T mahalanobisDistance = std::sqrt(
						delta.transpose() * PointMatcherSupport::inverseCovariance<T>(distributions[i].getCovariance(), distributions[i].getCovarianceEigenValues()) * delta);
