...
def parse_translation(p_translation: str, p_cloud_dim: int):
    ...
    p_translation = p_translation.strip('[]')
    p_translation = p_translation.replace(',', ' ')

    translation_values = np.fromiter(p_translation.split(' '), np.float64)
...
def parse_rotation(p_rotation: str, p_cloud_dim: int):
    ...
    p_rotation = p_rotation.strip('[]')
    p_rotation = p_rotation.replace(',', ' ')
    p_rotation = p_rotation.replace(';', ' ')

    rotation_matrix = np.fromiter(p_rotation.split(' '), np.float64)
