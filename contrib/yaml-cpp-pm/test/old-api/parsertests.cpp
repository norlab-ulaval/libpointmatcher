...
namespace Test
{
	namespace Parser {
		...
		bool CompressedMapAndSeq()
		{
			...
			const YAML_PM::Node& seq = doc["key"];
			if(seq.size() != 2)
				return false;
		...
		bool KeyNotFound()
		{
			...
			try {
				...
				if(e.msg != std::string(YAML_PM::ErrorMsg::KEY_NOT_FOUND) + ": bad key")
					throw;
			}
			...
			try {
				...
				if(e.msg != std::string(YAML_PM::ErrorMsg::KEY_NOT_FOUND) + ": 5")
					throw;
			}
			...
			try {
				...
				if(e.msg != std::string(YAML_PM::ErrorMsg::KEY_NOT_FOUND) + ": 2.5")
					throw;
			}
		...
		void PrepareNodeForTagExam(YAML_PM::Node& doc, const std::string& input)
		{
			std::stringstream stream(input);
			YAML_PM::Parser parser(stream);
			parser.GetNextDocument(doc);
		...
		bool ExpectedTagValue(YAML_PM::Node& node, const char* tag)
		{
			if(node.Tag() == tag)
			  return true;
        ...
        bool DoubleAsInt()
		{
            ...
            try {
                doc.to<int>();
            } catch(const YAML_PM::InvalidScalar& e) {
                return true;
            }
	...
	namespace {
		...
		void RunEncodingTest(EncodingFn encoding, bool declareEncoding, const std::string& name, int& passed, int& total)
		{
			...
			bool ok = true;
			try {
				YAML_PM::Parser parser(tester.stream());
				YAML_PM::Node doc;
				parser.GetNextDocument(doc);

				YAML_PM::Iterator itNode = doc.begin();
