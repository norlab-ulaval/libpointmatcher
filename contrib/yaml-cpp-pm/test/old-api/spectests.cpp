...
namespace Test {
	namespace Spec {
		...
		void operator >> (const YAML_PM::Node& node, Pair& p) {
			node[0] >> p.first;
			node[1] >> p.second;
		}
		...
		TEST InvalidEscapedCharacters()
		{
			...
			try {
				YAML_PM::Parser parser(stream);
				YAML_PM::Node doc;
				parser.GetNextDocument(doc);
			} catch(const YAML_PM::ParserException& e) {
				YAML_ASSERT(e.msg == std::string(YAML_PM::ErrorMsg::INVALID_ESCAPE) + "c");
				return true;
			}
		...
		void operator >> (const YAML_PM::Node& node, StringMap& m) {
			m._.clear();
			for(YAML_PM::Iterator it=node.begin();it!=node.end();++it) {
		...
		TEST InvalidRepeatedYAMLDirective()
		{
			try {
				...
				if(e.msg == YAML_PM::ErrorMsg::REPEATED_YAML_DIRECTIVE)
					return true;
		...
		TEST InvalidRepeatedTagDirective()
		{
			try {
				...
				if(e.msg == YAML_PM::ErrorMsg::REPEATED_TAG_DIRECTIVE)
					return true;
		...
		TEST NodeProperties()
		{
			PARSE(doc, ex6_23);
			YAML_ASSERT(doc.size() == 2);
			for(YAML_PM::Iterator it=doc.begin();it!=doc.end();++it) {
		...
		TEST InvalidTagShorthands()
		{
			...
			try {
				...
				threw = true;
				if(e.msg != YAML_PM::ErrorMsg::TAG_WITH_NO_SUFFIX)
					throw;
			}
		...
		TEST CompletelyEmptyNodes()
		{
			...
			YAML_ASSERT(doc[YAML_PM::Null].to<std::string>() == "bar");
			return true;
		}
		...
		TEST FlowMappingEntries()
		{
			...
			YAML_ASSERT(IsNull(doc[YAML_PM::Null]));
			return true;
		}
		...
		TEST FlowMappingSeparateValues()
		{
			...
			YAML_ASSERT(doc[YAML_PM::Null].to<std::string>() == "omitted key");
			return true;
		}
		...
		TEST SinglePairImplicitEntries()
		{
			...
			YAML_ASSERT(doc[1][0][YAML_PM::Null].to<std::string>() == "empty key entry");
			YAML_ASSERT(doc[2].size() == 1);
		...
		TEST InvalidImplicitKeys()
		{
			try {
				...
				if(e.msg == YAML_PM::ErrorMsg::END_OF_SEQ_FLOW)
					return true;
		...
		{
			{
				...
				try {
					...
					if(e.msg != YAML_PM::ErrorMsg::END_OF_SEQ)
						throw;
		...
		TEST BlockSequenceEntryTypes()
		{
			PARSE(doc, ex8_15);
			YAML_ASSERT(doc.size() == 4);
			YAML_ASSERT(YAML_PM::IsNull(doc[0]));
