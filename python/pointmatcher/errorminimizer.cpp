...
Typical error minimized are point-to-point and point-to-plane.
		...
		py::class_<ErrorElements>(pyErrorMinimizer, "ErrorElements", "A structure holding data ready for minimization. The data are \"normalized\", for instance there are no points with 0 weight, etc.")
			...
			.def(py::init<>())
			.def(py::init<const DataPoints&, const DataPoints&, const OutlierWeights&, const Matches&>(),
				 py::arg("requestedPts"), py::arg("sourcePts"), py::arg("outlierWeights"), py::arg("matches"));

		pyErrorMinimizer
			.def("getPointUsedRatio", &ErrorMinimizer::getPointUsedRatio)
			...
			.def("getOverlap", &ErrorMinimizer::getOverlap)
			.def("getCovariance", &ErrorMinimizer::getCovariance)
