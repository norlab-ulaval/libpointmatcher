...
i.e. for each node, the Visitor/Callback is called
		...
		firstptssamplerClass
			.def_readwrite("idx", &FirstPtsSampler::idx)
...
//			.def("__call__", &RandomPtsSampler::operator()<3>, py::arg("oc")); FIXME
		...
		py::enum_<SamplingMethod>(octreegridClass, "SamplingMethod")
			.value("FIRST_PTS", SamplingMethod::FIRST_PTS)
			.value("RAND_PTS", SamplingMethod::RAND_PTS)
			.value("CENTROID", SamplingMethod::CENTROID)
			.value("MEDOID", SamplingMethod::MEDOID)
			.export_values();

		octreegridClass
			.def_static("description", &OctreeGridDataPointsFilter::description)
